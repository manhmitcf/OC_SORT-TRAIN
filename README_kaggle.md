...
File cấu hình `exps/example/mot/custom_test.py` đã được tạo để chạy trên tập test.

```python
# Chạy đánh giá với model tốt nhất vừa train
!python tools/run_ocsort.py -f exps/example/mot/custom_test.py -c YOLOX_outputs/custom_finetune/best_ckpt.pth.tar --fp16 --fuse
```
